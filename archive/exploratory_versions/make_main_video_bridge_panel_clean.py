...
def normalize_video_sample(g: pd.DataFrame, smooth_win: int = 5) -> pd.DataFrame:
    ...
    # final agreed workflow: normalize_by p99, no running-max enforcement
...
def main():
    ...
    parser.add_argument("--out", type=Path, default=Path("main_video_bridge_panel_clean.png"))
    ...
    # agreed peak positions from final video metrics
    fapi_peak_ms = 46.0
    tempo_peak_ms = 170.0
    ...
    gs = GridSpec(
        2, 3,
        figure=fig,
        width_ratios=[1.0, 0.16, 1.0],   # narrow blank gutter in the middle
    ...
    ax_a.axvline(fapi_peak_ms, linestyle="--", linewidth=1.1, alpha=0.25, color="tab:blue")
    ax_a.axvline(tempo_peak_ms, linestyle="--", linewidth=1.1, alpha=0.25, color="tab:orange")
