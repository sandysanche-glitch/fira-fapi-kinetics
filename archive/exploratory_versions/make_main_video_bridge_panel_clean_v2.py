...
def main():
    ...
    parser.add_argument("--out", type=Path, default=Path("main_video_bridge_panel_clean_v2.png"))
    ...
    # agreed burst peak positions from final video metrics
    fapi_peak_ms = 46.0
    tempo_peak_ms = 170.0
    ...
    # wider middle gap, slightly narrower right side
    gs = GridSpec(
        2, 3,
        figure=fig,
        width_ratios=[1.00, 0.34, 0.90],
    ...
    # -------- left: video --------
    ax_a.plot(video["FAPI"]["time_ms"], video["FAPI"]["X_smooth"], linewidth=2.4, label="FAPI")
    ax_a.plot(video["FAPI-TEMPO"]["time_ms"], video["FAPI-TEMPO"]["X_smooth"], linewidth=2.4, label="FAPI-TEMPO")
    ax_a.set_ylabel("X (normalized)")
    ax_a.set_title("Video X(t)", fontsize=15, pad=6)
    style_axis(ax_a)
    ax_a.legend(frameon=False, loc="lower right", fontsize=11)

    # subtle peak guides in X panel
    ax_a.axvline(fapi_peak_ms, linestyle="--", linewidth=1.0, alpha=0.22, color="tab:blue")
    ax_a.axvline(tempo_peak_ms, linestyle="--", linewidth=1.0, alpha=0.22, color="tab:orange")

    ax_b.plot(video["FAPI"]["time_ms"], video["FAPI"]["dXdt"], linewidth=2.4, label="FAPI")
    ax_b.plot(video["FAPI-TEMPO"]["time_ms"], video["FAPI-TEMPO"]["dXdt"], linewidth=2.4, label="FAPI-TEMPO")
    ax_b.set_xlabel("Time (ms)")
    ax_b.set_ylabel("dX/dt (per ms)")
    ax_b.set_title("Video dX/dt", fontsize=15, pad=6)
    style_axis(ax_b)
    ...
    # -------- right: bridge --------
    ax_c.plot(bridge["FAPI"]["time_ms"], bridge["FAPI"]["X"], linewidth=2.4, label="FAPI")
    ax_c.plot(bridge["FAPI-TEMPO"]["time_ms"], bridge["FAPI-TEMPO"]["X"], linewidth=2.4, label="FAPI-TEMPO")
    ax_c.set_ylabel("Transformed fraction")
    ax_c.set_title("Bridge X(t)", fontsize=15, pad=6)
    style_axis(ax_c)
    ax_c.legend(frameon=False, loc="lower right", fontsize=11)

    ax_d.plot(bridge["FAPI"]["time_ms"], bridge["FAPI"]["dX_dt"], linewidth=2.4, label="FAPI")
    ax_d.plot(bridge["FAPI-TEMPO"]["time_ms"], bridge["FAPI-TEMPO"]["dX_dt"], linewidth=2.4, label="FAPI-TEMPO")
    ax_d.set_xlabel("Effective time (ms)")
    ax_d.set_ylabel("Transformation rate")
    ax_d.set_title("Bridge dX/dt(t)", fontsize=15, pad=6)
    style_axis(ax_d)
    ...
    # a bit more breathing room left/right/top/bottom
    fig.subplots_adjust(left=0.055, right=0.99, top=0.95, bottom=0.11)
