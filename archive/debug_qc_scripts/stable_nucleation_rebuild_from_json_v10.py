...
"""
stable_nucleation_rebuild_from_json_v10.py

ROBUST + CORRECT handling of empty previous frame for overlap gating.

Key fix vs v8/v9:
- If previous frame JSON is missing/empty -> overlap_prev is set to 0.0 (configurable)
  instead of NaN. This avoids rejecting everything when detections start later (e.g. frame 18).

Still robust:
- overlap computed in subprocess with per-call timeout (won’t hang)
- optional strict overlap (reject only when overlap truly unknown due to timeout/kill/etc.)
...
def compute_overlap_prev(
    ...
    """
    Returns (overlap_prev, note, n_used).
    note can be:
      - prev_empty_or_missing
      - prev_no_candidates_nearby
      - prev_no_valid_rle
      - overlap_total_timeout(...)
      - overlap_all_nan(...)
      - ok / ok(killed=...)
    """
    if not prev_anns:
...
def main():
    ...
    # ---- NEW: how to treat missing/empty prev frame ----
    ap.add_argument("--accept_empty_prev", action="store_true",
                    help="If prev frame JSON is missing/empty, treat overlap_prev as empty_prev_overlap_value.")
    ap.add_argument("--empty_prev_overlap_value", type=float, default=0.0,
                    help="Overlap value to use when prev frame has no masks (default 0).")
    ap.add_argument("--accept_no_candidate_prev", action="store_true",
                    help="If prev frame has masks but none nearby, treat overlap_prev as no_candidate_overlap_value.")
    ...
    for i_local, (tid, df) in enumerate(slice_groups, start=1):
        ...
        if args.overlap_prev_max < 0.999:
            ...
            overlap_prev, overlap_note, n_used = compute_overlap_prev(
                cur_ann=ann,
                cur_rle=cur_rle,
                prev_anns=prev_anns,
            ...
            # ---- FIX: treat empty prev as 0 instead of NaN ----
