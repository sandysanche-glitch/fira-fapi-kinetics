#!/usr/bin/env python
"""
compare_SI_from_json_KXinf_fixedn.py

Directly read FAPI / FAPI-TEMPO JSON detection files (Mask R-CNN-style),
compute SI metrics, and fit Avrami *K* and *X_inf* for both datasets with a
fixed exponent n:

    X(t) = X_inf * (1 - exp(-K * t^n))

n is fixed (typically 2.5); K and X_inf are fitted by least squares to X_pred(t).
...
# ---- SciPy (for dn/dt and convenience) ----
try:
...
def eff_growth_rate_um_per_ms(R_um: np.ndarray, dt_ms: np.ndarray,
    ...
    For now circularity and defect_frac are trivial (1, 0) unless you plug in real values.
    """
...
def fit_dn_dt(t0_ms: np.ndarray, area_um2: float, grid_ms: np.ndarray):
    ...
    """
    try:
        from scipy.stats import lognorm, gamma  # ensure available
        use_scipy = HAVE_SCIPY
    except Exception:
        use_scipy = False

    tpos = t0_ms[(t0_ms > 0) & np.isfinite(t0_ms)]
    if len(tpos) < 5 or not use_scipy:
...
# ---- Fit K and X_inf with fixed n ----

def fit_K_Xinf_from_X(t_ms: np.ndarray, X_pred: np.ndarray, n: float,
                      x_min: float = 0.05, x_max: float = 0.95):
    """
    Fit K and X_inf in generalized Avrami:

        X_model(t) = X_inf * (1 - exp(-K * t^n))

    with fixed n, by least squares vs X_pred(t).

    We restrict to X range x_min <= X <= x_max to avoid noisy tails.
    Time is in ms → K has units of ms^-n.
    """
    t_ms = np.asarray(t_ms, dtype=float)
    X_pred = np.asarray(X_pred, dtype=float)

    # select mid-range of X
    y = np.clip(X_pred, 0.0, 1.0)
    valid = (t_ms > 0) & (y > x_min) & (y < x_max)
    t = t_ms[valid]
    yv = y[valid]

    if t.size < 5:
        return 1.0, 0.0  # X_inf ~1, K ~0

    # initial guesses
    Xinf0 = float(np.nanmax(yv))
    if Xinf0 <= 0:
        Xinf0 = 1.0
    # use normalized y for analytic K0 guess
    y_norm = np.clip(yv / Xinf0, 1e-9, 0.999999)
    t_pow = t ** n
    with np.errstate(all='ignore'):
        K_vals = -np.log(1.0 - y_norm) / t_pow
    ...
    def sse(params):
        ...
        K = np.exp(logK)
        Xinf = np.exp(logXinf)
        X_model = Xinf * (1.0 - np.exp(-K * (t ** n)))
        return float(np.sum((X_model - yv) ** 2))

    # bounds in log-space to keep positivity
    logK0 = math.log(max(K0, 1e-12))
    logX0 = math.log(max(Xinf0, 1e-9))

    if HAVE_SCIPY:
        # small box around initial guess
        bounds = [
            (logK0 - 5, logK0 + 5),
            (math.log(1e-6), math.log(10.0 * max(1.0, Xinf0)))
        ]
        res = minimize(sse, x0=[logK0, logX0], bounds=bounds, method="L-BFGS-B")
        if res.success:
            logK_opt, logX_opt = res.x
            K_opt = float(np.exp(logK_opt))
            Xinf_opt = float(np.exp(logX_opt))
            return Xinf_opt, K_opt

    # fallback: coarse grid search in log-space
    logK_vals = np.linspace(logK0 - 3, logK0 + 3, 60)
    logX_vals = np.linspace(math.log(0.2 * Xinf0), math.log(2.0 * Xinf0 + 1e-6), 40)
    best_err = np.inf
    best_K = K0
    best_Xinf = Xinf0
    for lk in logK_vals:
        for lx in logX_vals:
            ...
            Xinf = math.exp(lx)
            X_model = Xinf * (1.0 - np.exp(-K * (t ** n)))
            err = float(np.sum((X_model - yv) ** 2))
            if err < best_err:
                best_err = err
                best_K = K
                best_Xinf = Xinf

    return best_Xinf, best_K
...
def compute_si_from_areas(area_px: np.ndarray,
    ...
    Returns:
        ...
        Xinf_fit, K_fit: fitted Avrami parameters
    """

    # Penalties: for now C=1, phi=0 for all objects, but framework is here
    C = np.ones_like(area_px, dtype=float)
    phi = np.zeros_like(area_px, dtype=float)

    # nucleation times from ranks
    ...
    # Fit X_inf and K with fixed n
    n = float(n_avrami)
    Xinf_fit, K_fit = fit_K_Xinf_from_X(t_ms, X_pred, n=n,
                                        x_min=x_min_fit, x_max=x_max_fit)
    X_A = Xinf_fit * (1.0 - np.exp(-K_fit * (t_ms ** n)))
    ...
    X_df = pd.DataFrame({
        ...
        "X_Avrami": X_A,
        "X_inf_fit": Xinf_fit,
        "K_fit_ms_units": K_fit,
    })

    return out_obj_df, dndt_df, X_df, Xinf_fit, K_fit
...
def main():
    ap = argparse.ArgumentParser(
        description="Compare FAPI vs FAPI-TEMPO from JSON RLE masks, Avrami K & X_inf LS fit with fixed n."
    )
    ap.add_argument(
        "--fapi_dir",
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.05,
        help="Lower X threshold for fitting K & X_inf (e.g. 0.05 = 5% transformed).",
    )
    ap.add_argument(
        "--x_max_fit",
        type=float,
        default=0.95,
        help="Upper X threshold for fitting K & X_inf (e.g. 0.95 = 95% transformed).",
    )
    ap.add_argument(
        "--min_score",
        type=float,
    ...
    outA, dnA, XA, Xinf_A, K_A = compute_si_from_areas(
        areaA, labelA,
    ...
    outB, dnB, XB, Xinf_B, K_B = compute_si_from_areas(
        areaB, labelB,
        px_per_um=args.px_per_um,
        area_eff_px=area_eff_px,
        n_avrami=args.n_avrami,
