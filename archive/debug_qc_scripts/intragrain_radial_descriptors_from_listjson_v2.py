#!/usr/bin/env python
"""
Radial intragrain descriptors from list-style JSON + heat-map / optical images.

Each JSON is a list of dicts like:
  {"bbox": [...],
   "score": ...,
   "segmentation": {"size": [H, W], "counts": "..."},
   "mask_name": "FAPI_0_0.png"}

We ignore mask_name and decode the COCO RLE in "segmentation".
"""
...
try:
    ...
    raise ImportError(
        "pycocotools is required. Install it with:\n"
...
def process_dataset(
    ...
    """
    For each JSON file in dir_path, use list of instances with COCO RLE
    and one intensity image per field to compute radial profiles.
    ...
    for jf in json_files:
        ...
        if not img_path.exists():
            print(f"[WARN]   no intensity image for {base}; skipping whole file")
            continue
        ...
        for entry in entries:
            ...
            try:
                ...
                print(f"[WARN]   RLE decode failed in {jf.name}: {e}")
                continue

            # sanity: crop to image size if needed
            H, W = intensity.shape
            if mask.shape != (H, W):
                # resize / crop if size mismatch (shouldn't usually happen)
                h2 = min(H, mask.shape[0])
                w2 = min(W, mask.shape[1])
                mask = mask[:h2, :w2]
                m2 = np.zeros_like(intensity, dtype=bool)
                m2[:h2, :w2] = mask
                mask = m2
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Radial intragrain descriptors from list JSON + COCO RLE + images"
    )
...
def main():
    ...
    plot_two_profiles(
        ...
        base.parent / (base.name + "_radial_intensity_profiles.png"),
    )
    ...
    plot_two_profiles(
        ...
        base.parent / (base.name + "_radial_entropy_profiles.png"),
    )

    # 3) radial texture anisotropy
    plot_two_profiles(
        r,
        A_m_fapi,
        A_s_fapi,
        ...
        base.parent / (base.name + "_radial_anisotropy_profiles.png"),
    )
