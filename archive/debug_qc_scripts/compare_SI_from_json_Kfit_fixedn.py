#!/usr/bin/env python
"""
compare_SI_from_json_Kfit_fixedn.py

Directly read FAPI / FAPI-TEMPO JSON detection files (Mask R-CNN-style),
compute SI metrics, fit Avrami K for both datasets with a fixed exponent n,
...
# ---- SciPy (for dn/dt and optionally K fitting search convenience) ----
try:
...
# ---- K fitting with fixed n, least squares on X_pred(t) ----

def fit_K_from_X(t_ms: np.ndarray, X_pred: np.ndarray, n: float,
                 x_min: float = 0.05, x_max: float = 0.95) -> float:
    """
    Fit K in Avrami law: X_A(t) = 1 - exp(-K * t^n)
    by minimizing squared error between X_pred and the model, for a fixed n.
    ...
    # quick analytic estimate (original median estimator) as initial guess
    t_pow = t ** n
    with np.errstate(all='ignore'):
    ...
    # search bounds in log-space around K0
    if K0 <= 0:
        K0 = 1e-6
    ...
    Ks = np.logspace(np.log10(lo), np.log10(hi), 200)
...
def compute_si_from_areas(area_px: np.ndarray,
    ...
    Returns:
        ...
        K_fit: fitted Avrami K for fixed n
    """
    ...
    # Fit K with fixed n
    n = float(n_avrami)
    K_fit = fit_K_from_X(t_ms, X_pred, n=n, x_min=x_min_fit, x_max=x_max_fit)
...
def main():
    ap = argparse.ArgumentParser(
        description="Compare FAPI vs FAPI-TEMPO from JSON RLE masks, Avrami K LS fit with fixed n."
    )
    ap.add_argument(
        "--fapi_dir",
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.05,
        help="Lower X threshold for K fit (e.g. 0.05 = 5% transformed).",
    )
    ap.add_argument(
        "--x_max_fit",
        type=float,
        default=0.95,
        help="Upper X threshold for K fit (e.g. 0.95 = 95% transformed).",
    )
    ap.add_argument(
        "--min_score",
        type=float,
    ...
    # 4) Combined K fits
    pd.DataFrame([
        {"label": labelA, "n_avrami": args.n_avrami,
         "x_min_fit": args.x_min_fit, "x_max_fit": args.x_max_fit,
         "K_fit_ms_units": K_A},
        {"label": labelB, "n_avrami": args.n_avrami,
         "x_min_fit": args.x_min_fit, "x_max_fit": args.x_max_fit,
         "K_fit_ms_units": K_B},
    ...
    try:
        ...
        if vA.size > 0 or vB.size > 0:
            # recompute bins same way as above for plotting
            if vA.size > 0 and vB.size > 0:
                v_all = np.concatenate([vA, vB])
            else:
                v_all = vA if vA.size > 0 else vB
