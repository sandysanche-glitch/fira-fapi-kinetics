...
"""
stable_nucleation_rebuild_from_json_v9.py

ROBUST stable nucleation rebuild + overlap gating from frame JSONs.

v9 changes vs v8:
- overlap_strategy: 'closest' / 'kclosest' / 'max'
  TEMPO needs 'closest' or 'kclosest' to avoid rejecting due to unrelated nearby masks.
- still uses safe multiprocessing overlap to prevent C-level hangs.
"""
...
def compute_overlap_prev_robust(
    ...
    overlap_strategy:
      - 'closest'  : overlap only vs closest prev candidate
      - 'kclosest' : overlap vs K closest candidates and take max
      - 'max'      : overlap vs all candidates (up to max_candidates) and take max (old behavior)
    """
    if not prev_anns:
    ...
    cand.sort(key=lambda x: x[0])  # nearest first

    # candidate selection by strategy
...
def main():
    ...
    ap.add_argument("--overlap_strategy", choices=["closest", "kclosest", "max"], default="kclosest",
                    help="TEMPO recommended: closest or kclosest. 'max' is very conservative.")
    ...
    ap.add_argument("--track_timeout_s", type=float, default=15.0)
