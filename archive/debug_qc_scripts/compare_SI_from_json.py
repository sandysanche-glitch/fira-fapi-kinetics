#!/usr/bin/env python
"""
compare_SI_from_json.py

Directly read FAPI / FAPI-TEMPO JSON detection files (Mask R-CNN-style),
compute SI metrics, fit Avrami K for both, and export combined CSVs and plots.
...
def fit_K_from_X(t_ms: np.ndarray, X_pred: np.ndarray, n: float) -> float:
    ...
    using median estimator on valid points.
    """
...
def collect_areas_from_json_folder(folder: Path, min_score: float = 0.0) -> np.ndarray:
    ...
    for jf in json_files:
        ...
        for obj in data:
            ...
            # seg should be {'size': [H, W], 'counts': <RLE>}
            try:
                rle = {
                    "size": seg["size"],
                    "counts": seg["counts"]
...
def compute_si_from_areas(area_px: np.ndarray,
    ...
    # Per-object metrics (like *_SI_metrics.csv)
...
def main():
    ap = argparse.ArgumentParser(description="Compare FAPI vs FAPI-TEMPO directly from JSON RLE masks.")
    ap.add_argument(
        "--fapi_dir",
    ...
    ap.add_argument(
        ...
        type=float,
        default=2.20014,
        help="Pixels per micron (same as in export_SI_metrics_rank_v2.py)",
    )
    ap.add_argument(
        "--area_eff_px",
        type=float,
        ...
        help="Effective analyzed area in pixels. "
             "If not set, estimated as full image area from first JSON.",
    )
    ap.add_argument(
        "--n_avrami",
        type=float,
    ...
    ap.add_argument(
        "--use_penalties",
        action="store_true",
        help="Enable penalties based on circularity/defect_frac (here they are just 1 and 0).",
    )
    ...
    # 4) Combined K fits
    pd.DataFrame([
        {"label": labelA, "n_avrami": args.n_avrami, "K_fit": K_A},
        {"label": labelB, "n_avrami": args.n_avrami, "K_fit": K_B},
