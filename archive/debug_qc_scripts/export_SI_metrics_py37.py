...
def growth_speed_mps_from_ray(
    df: pd.DataFrame,
    v0_px_per_ms: Optional[float] = None,
    alpha: float = 0.0,
    beta: float = 0.0,
...
def main():
    ...
    ap.add_argument("--objects_csv", required=True,
                    help="Per-object table CSV with at least: ray_px, t0_ms, area_px, perim_px. Optional: defect_area_px, circ, defect_frac.")
    ...
    if missing:
        raise ValueError("Missing required columns in objects CSV: {}".format(sorted(missing)))
