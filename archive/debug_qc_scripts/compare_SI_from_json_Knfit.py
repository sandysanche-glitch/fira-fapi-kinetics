#!/usr/bin/env python
"""
compare_SI_from_json_Knfit.py

Directly read FAPI / FAPI-TEMPO JSON detection files (Mask R-CNN-style),
compute SI metrics, fit Avrami n and K for both (log-log Avrami fit),
...
# ---- improved Avrami fit: fit both n and K ----

def fit_nK_from_X(t_ms: np.ndarray, X_pred: np.ndarray,
                  x_min: float = 0.05, x_max: float = 0.95):
    """
    Fit n and K in Avrami law:  X(t) = 1 - exp(-K * t^n)
    using a linear regression in Avrami coordinates:

        ln(-ln(1 - X)) = ln K + n ln t

    We restrict to a mid-range of X: x_min <= X <= x_max to
    avoid noisy early times and saturated region.

    Time is in ms, so K has units of ms^-n.
    """
    t_ms = np.asarray(t_ms, dtype=float)
    X_pred = np.asarray(X_pred, dtype=float)

    # clip to (0, 1) but keep mid-range window
    y = np.clip(X_pred, 1e-6, 1 - 1e-6)
    valid = (t_ms > 0) & (y > x_min) & (y < x_max)
    t = t_ms[valid]
    xv = y[valid]

    if t.size < 5:
        # too few points, bail out
        return 2.0, 0.0  # some default n, K

    # Avrami transform
    with np.errstate(all='ignore'):
        Y = np.log(-np.log(1.0 - xv))   # ln(-ln(1 - X))
        Xlog = np.log(t)                # ln(t)
    ...
    # Linear regression: Y = a + b Xlog  →  a = lnK, b = n
    b, a = np.polyfit(Xlog, Y, 1)
    n_fit = float(b)
    K_fit = float(np.exp(a))
    return n_fit, K_fit
...
def compute_si_from_areas(area_px: np.ndarray,
    ...
    Returns:
        ...
        n_fit, K_fit: fitted Avrami parameters
    """
    ...
    # Fit n and K from X_pred(t)
    n_fit, K_fit = fit_nK_from_X(t_ms, X_pred, x_min=x_min_fit, x_max=x_max_fit)
    X_A = 1.0 - np.exp(-K_fit * (t_ms ** n_fit))
    ...
    return out_obj_df, dndt_df, X_df, n_fit, K_fit
...
def main():
    ap = argparse.ArgumentParser(
        description="Compare FAPI vs FAPI-TEMPO directly from JSON RLE masks, Avrami n & K fitted from X_pred."
    )
    ap.add_argument(
        "--fapi_dir",
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.05,
        help="Lower X threshold for Avrami fit (e.g. 0.05 = 5% transformed).",
    )
    ap.add_argument(
        "--x_max_fit",
        type=float,
        default=0.95,
        help="Upper X threshold for Avrami fit (e.g. 0.95 = 95% transformed).",
    )
    ap.add_argument(
        "--min_score",
        type=float,
    ...
    outA, dnA, XA, n_A, K_A = compute_si_from_areas(
        areaA, labelA, px_per_um=args.px_per_um, area_eff_px=area_eff_px,
        x_min_fit=args.x_min_fit, x_max_fit=args.x_max_fit,
        use_penalties=args.use_penalties,
        alpha=args.alpha, beta=args.beta
    )
    outB, dnB, XB, n_B, K_B = compute_si_from_areas(
    ...
    # 4) Combined n, K fits
    pd.DataFrame([
        {"label": labelA, "n_fit": n_A, "K_fit_ms_units": K_A},
        {"label": labelB, "n_fit": n_B, "K_fit_ms_units": K_B},
