...
def normalize_key(x: str) -> str:
    ...
    x_low = x.lower()
    if x_low.endswith(".png"):
        x = x[:-4]
    elif x_low.endswith(".jpg"):
        x = x[:-4]
    elif x_low.endswith(".jpeg"):
        x = x[:-5]
    elif x_low.endswith(".json"):
        x = x[:-5]
    return x
...
def compute_mask_geometry(mask: np.ndarray):
    ...
    req_px = float(np.sqrt(area_px / np.pi))

    return {
        "centroid_x_px": float(cx),
        "centroid_y_px": float(cy),
        "area_px": area_px,
        "equiv_radius_px": req_px,
    }
...
def extract_geometry_from_json_dir(
    ...
    for jf in json_files:
        ...
        image_id = normalize_key(jf.stem)

        for i, ann in enumerate(data):
            n_entries += 1

            if int(ann.get("category_id", -999)) != crystal_category_id:
                continue
            n_crystal_entries += 1
            ...
            mask_name = ann.get("mask_name", f"{image_id}_{i}")
            ...
            if int(mask_bin.sum()) < min_pixels:
                continue
            ...
            rows.append(
                {
                    "json_file": str(jf),
                    "image_id": image_id,
                    "grain_index_in_json": i,
                    "mask_name": normalize_key(mask_name),
                    "category_id": crystal_category_id,
                    **geom,
                }
            )
            n_used += 1
    ...
    if not rows:
        raise RuntimeError(f"No usable crystal masks found in {json_dir}")

    df = pd.DataFrame(rows)

    # NN within each image only
    ...
    info = {
        ...
        "n_entries_category1": n_crystal_entries,
        "n_crystal_masks_used": n_used,
    }
...
def merge_geometry_with_veff(geom_df: pd.DataFrame, veff_df: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    g = geom_df.copy()
    ...
    n_total = len(merged)
    n_match = int(merged["nn_distance_px"].notna().sum())
    info = {
        "n_rows_veff": n_total,
        "n_rows_with_geometry_match": n_match,
        "geometry_match_fraction": float(n_match / n_total) if n_total else np.nan,
    }
    return merged, info
...
def main():
    ap = argparse.ArgumentParser(
        description="Compute local crowding/impingement from JSON crystal masks (category_id=1) and merge to with_veff."
    )
    ...
    print("[INFO] Extracting FAPI crystal geometry from JSON...")
    geom_fapi, info_fapi_geom = extract_geometry_from_json_dir(
        Path(args.fapi_json_dir),
        min_pixels=args.min_pixels,
        crystal_category_id=args.crystal_category_id,
    )

    print("[INFO] Extracting FAPI-TEMPO crystal geometry from JSON...")
    geom_tempo, info_tempo_geom = extract_geometry_from_json_dir(
        Path(args.tempo_json_dir),
        min_pixels=args.min_pixels,
        crystal_category_id=args.crystal_category_id,
    )

    geom_fapi_out = out_dir / f"{base}_FAPI_geometry_from_json.csv"
    geom_tempo_out = out_dir / f"{base}_FAPITEMPO_geometry_from_json.csv"
    geom_fapi.to_csv(geom_fapi_out, index=False)
    geom_tempo.to_csv(geom_tempo_out, index=False)

    print(f"[OK] Exported geometry: {geom_fapi_out}")
    print(f"[OK] Exported geometry: {geom_tempo_out}")
    ...
    fapi_m, info_fapi_merge = merge_geometry_with_veff(geom_fapi, fapi)
    tempo_m, info_tempo_merge = merge_geometry_with_veff(geom_tempo, tempo)
    ...
    print(f"[OK] Exported merged table: {fapi_out}")
    print(f"[OK] Exported merged table: {tempo_out}")
