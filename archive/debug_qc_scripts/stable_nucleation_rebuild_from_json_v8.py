...
"""
stable_nucleation_rebuild_from_json_v8.py

ROBUST stable nucleation rebuild with overlap gating from frame JSONs.

Why v8:
- Your run can "freeze" inside pycocotools C calls (merge/area). Python timeouts can't interrupt that.
- v8 computes overlap in a separate process with a timeout; if it hangs, we kill the worker and keep going.
- Also adds per-track timeout and continuous checkpoint/state writes.

Inputs:
  --tracks_csv : tracks.csv
  --json_dir   : folder of frame_XXXXX_tYY.YYms_idmapped.json
...
Resume:
  --resume   will read out_dir/state.json and continue from next track index.
"""
...
def normalize_rle(seg: Any) -> Optional[Dict[str, Any]]:
    ...
    if "counts" not in seg or "size" not in seg:
        return None
    rle = dict(seg)
    # pycocotools expects counts as bytes sometimes
...
def get_first_usable_match(json_dir: str, start_frame: int, lookahead_k: int,
    ...
    """
    Find a usable detection around nuc_frame.
    If nuc frame JSON is empty, look ahead up to k frames.
    """
...
# -------------------------
# Overlap computation (SAFE)
# -------------------------
def _overlap_worker(q, cur_rle, prev_rle):
    """Runs in a separate process. Any hang stays in the child."""
    try:
...
def overlap_frac_safe_mp(cur_rle: Dict[str, Any],
    ...
    """
    Compute overlap fraction in a subprocess so it cannot hang the main run.
    Returns (overlap, note).
    """
    ...
    try:
        ...
        return float("nan"), "overlap_no_queue_value"
...
def compute_overlap_prev_robust(
    ...
    - Total wall-clock budget across candidates
    - Each candidate overlap is computed safely (subprocess) if use_safe_mp=True
    """
    if not prev_anns:
    ...
    for prle in prev_rles:
        if timeout_s_total > 0 and (time.time() - t0) > timeout_s_total:
            return float("nan"), f"overlap_total_timeout(killed={killed})", len(prev_rles)
        ...
        if use_safe_mp:
            ...
            # WARNING: can hang on pathological RLEs
            try:
                inter = mask_utils.merge([cur_rle, prle], intersect=True)
                ai = float(mask_utils.area(inter))
                aa = float(mask_utils.area(cur_rle))
                of = ai / aa if aa > 0 else float("nan")
            except Exception:
                of = float("nan")
    ...
    if not got_any:
        return float("nan"), f"overlap_all_nan(killed={killed})", len(prev_rles)

    return best, (f"ok(killed={killed})" if killed > 0 else "ok"), len(prev_rles)
...
def main():
    ...
    ap.add_argument("--strict_overlap", action="store_true",
                    help="If overlap cannot be computed (NaN/timeout/killed), reject.")
    ap.add_argument("--reject_if_overlap_nan", action="store_true",
                    help="Older flag: reject if overlap is NaN (same intent as strict_overlap).")

    # Lookahead for empty nuc-frame JSONs
    ...
    # Previous-frame candidate search
    ap.add_argument("--prev_search_pad_px", type=float, default=250.0)
    ap.add_argument("--prev_max_candidates", type=int, default=50)

    # Overlap time budgets
    ap.add_argument("--overlap_timeout_s", type=float, default=0.5,
                    help="Total time budget per track for overlap scan across candidates.")
    ap.add_argument("--overlap_per_call_timeout_s", type=float, default=0.05,
                    help="Timeout per candidate overlap call (safe-mp mode).")
    ap.add_argument("--no_overlap_safe_mp", action="store_true",
                    help="Disable safe multiprocessing overlap (NOT recommended).")

    # Track time budget
    ap.add_argument("--track_timeout_s", type=float, default=10.0,
                    help="Hard cap per track; if exceeded, auto-skip and continue.")
    ...
    ap.add_argument("--resume", action="store_true",
                    help="Resume from out_dir/state.json if present.")
    ap.add_argument("--state_path", default=None,
                    help="Optional custom state path (default: out_dir/state.json).")
    ...
    # resume overrides start_track_idx
    ...
    slow_fields = [
        "global_i", "track_id", "mask_frame_i", "note",
        "pad_px", "max_candidates", "timeout_s_total", "timeout_s_call", "n_candidates_used"
    ]
    ...
    use_safe_mp = (not args.no_overlap_safe_mp)
    ...
    for i_local, (tid, df) in enumerate(slice_groups, start=1):
        ...
        # periodic state write (at start, so even if it hangs later you know where)
        write_state(state_path, {
            "updated_unix_s": time.time(),
            "elapsed_run_s": time.time() - t_run0,
            "current_global_i": i_global,
            "current_track_id": int(tid),
            "next_start_track_idx": i_global - 1,  # conservative until track finishes
        })

        n_obs = len(df)

        # hard per-track timeout checks (sprinkled below as well)
        def track_timed_out() -> bool:
            return (args.track_timeout_s > 0) and ((time.time() - t_track0) > float(args.track_timeout_s))

        # Track too short
        if n_obs < args.L:
            rejected.append({"track_id": int(tid), "reason": f"too_short(n={n_obs}<L={args.L})"})
            # finished -> update state to next
            write_state(state_path, {
                "updated_unix_s": time.time(),
                "elapsed_run_s": time.time() - t_run0,
                "current_global_i": i_global,
                "current_track_id": int(tid),
                "next_start_track_idx": i_global,  # next one
            })
            continue
        ...
        for j in range(0, n_obs - args.L + 1):
            if track_timed_out():
                rejected_trackgate.append({"track_id": int(tid), "reason": "track_timeout_during_stable_run_search"})
                nuc_i = None
                break
        ...
        if nuc_i is None:
            if not any(r.get("track_id") == int(tid) for r in rejected_trackgate[-3:]):
        ...
        if track_timed_out():
            rejected_trackgate.append({"track_id": int(tid), "reason": "track_timeout_pre_match"})
            write_state(state_path, {
                "updated_unix_s": time.time(),
                "elapsed_run_s": time.time() - t_run0,
                "current_global_i": i_global,
        ...
        if args.overlap_prev_max < 0.999:
            if track_timed_out():
                rejected_trackgate.append({"track_id": int(tid), "reason": "track_timeout_pre_overlap"})
                write_state(state_path, {
                    "updated_unix_s": time.time(),
                    "elapsed_run_s": time.time() - t_run0,
                    "current_global_i": i_global,
            ...
            if ("timeout" in overlap_note) or ("killed" in overlap_note):
                append_log_row(slow_log_path, {
                    ...
                    "max_candidates": int(args.prev_max_candidates),
                    "timeout_s_total": float(args.overlap_timeout_s),
                    "timeout_s_call": float(args.overlap_per_call_timeout_s),
        ...
        # finished track -> update state to next
        write_state(state_path, {
            "updated_unix_s": time.time(),
            "elapsed_run_s": time.time() - t_run0,
            "current_global_i": i_global,
...
if __name__ == "__main__":
    # On Windows, multiprocessing spawn needs this guard.
    main()
