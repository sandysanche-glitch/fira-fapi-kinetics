...
from scipy.ndimage import distance_transform_edt, center_of_mass, uniform_filter
...
def local_defect_density_map(defect_mask: np.ndarray | None, window: int = 15) -> np.ndarray | None:
    if defect_mask is None:
        return None
    arr = defect_mask.astype(np.float32)
    dens = uniform_filter(arr, size=window, mode="nearest")
    return dens.astype(np.float32)


def gradient_magnitude_map(img: np.ndarray) -> np.ndarray:
...
def sample_dataset(
    ...
    for jf in sorted(json_dir.glob("*.json")):
        ...
        gradmag = gradient_magnitude_map(heatmap)
        stem = jf.stem
        ...
        defect_dens = local_defect_density_map(defect_union, defect_density_window) if defect_union is not None else None
        ...
        for ann_idx, ann in crystals:
            ...
            defdens_map = defect_dens if defect_dens is not None and defect_dens.shape == crystal_mask.shape else None
                ...
                rows.append(
                    {
                        ...
                        "json_stem": stem,
                        "file_name": grain_key,
                        "defect_density_local": float(defdens_map[y, x]) if defdens_map is not None else np.nan,
                        "gradient_magnitude_local": float(gradmag[y, x]),
...
def main():
    ap = argparse.ArgumentParser(description="Sample local defect density and gradient magnitude against local fields.")
    ...
    ap.add_argument("--defect-density-window", type=int, default=15)
    ...
    fapi = sample_dataset(
        ...
        args.min_crystal_pixels, args.max_pixels_per_grain, args.defect_density_window
    )
    ...
    if args.save_raw:
        fapi.to_csv(out_dir / "defectdensity_gradient_vs_local_fields_raw_FAPI.csv", index=False)
        tempo.to_csv(out_dir / "defectdensity_gradient_vs_local_fields_raw_FAPITEMPO.csv", index=False)

    descriptors = [
        ("dist_boundary_px", "Distance to boundary (px)", "boundary"),
        ("dist_defect_px", "Distance to defect (px)", "defect"),
        ("dist_nucleus_px", "Distance to nucleus (px)", "nucleus"),
    ]
    observables = [
        ("defect_density_local", "Local defect density", "defect_density"),
        ("gradient_magnitude_local", "Heatmap gradient magnitude", "gradient_magnitude"),
    ]

    for ycol, ylabel, ytag in observables:
        for xcol, xlabel, xtag in descriptors:
            fb = binned_stats(fapi, xcol, ycol, args.nbins)
            tb = binned_stats(tempo, xcol, ycol, args.nbins)
            fb.insert(0, "dataset", "FAPI")
            tb.insert(0, "dataset", "FAPI-TEMPO")
            pd.concat([fb, tb], ignore_index=True).to_csv(out_dir / f"binned_{ytag}_vs_{xtag}.csv", index=False)
            plot_two(
                fb, tb, xlabel, ylabel,
                f"{ylabel} vs {xlabel.lower()}",
                out_dir / f"binned_{ytag}_vs_{xtag}.png"
            )

    print("[OK] Wrote defect-density / gradient-vs-local-fields outputs to", out_dir)
