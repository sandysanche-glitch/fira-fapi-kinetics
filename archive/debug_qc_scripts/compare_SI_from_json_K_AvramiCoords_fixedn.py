#!/usr/bin/env python
"""
compare_SI_from_json_K_AvramiCoords_fixedn.py
...
n is fixed (typically 2.5); K and X_inf are fitted in Avrami coordinates.
...
# ---- Fit K and X_inf in Avrami coordinates (fixed n) ----
...
def fit_K_Xinf_from_X_avrami_coords(t_ms: np.ndarray, X_pred: np.ndarray, n: float,
    ...
    We:
        - take X_inf = max(X_pred) (so saturation matches your curve)
    ...
    y = np.clip(X_pred / X_inf, 1e-9, 1.0 - 1e-9)
    ...
    # With slope fixed to n, intercept = ln K is just the average residual
...
def compute_si_from_areas(area_px: np.ndarray,
    ...
    # Fit X_inf and K with fixed n in Avrami coordinates
    n = float(n_avrami)
    Xinf_fit, K_fit = fit_K_Xinf_from_X_avrami_coords(
...
def main():
    ap = argparse.ArgumentParser(
        description="Compare FAPI vs FAPI-TEMPO from JSON RLE masks, Avrami K & X_inf fit in Avrami coordinates (fixed n)."
    )
    ap.add_argument(
        "--fapi_dir",
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.05,
        help="Lower X threshold (relative to X_inf) for fitting K & X_inf (e.g. 0.05 = 5% transformed).",
    )
    ap.add_argument(
        "--x_max_fit",
        type=float,
        default=0.95,
        help="Upper X threshold (relative to X_inf) for fitting K & X_inf (e.g. 0.95 = 95% transformed).",
    )
    ap.add_argument(
        "--min_score",
        type=float,
    ...
    try:
        ...
        plt.plot(
            X_comb["t_s"], X_comb[f"X_Avrami_{labelA}"],
            linestyle="--",
            label=f"{labelA} Avrami (n={n:.2f}, X_inf={Xinf_A:.2f}, K={K_A:.3g})",
        )
        plt.plot(X_comb["t_s"], X_comb[f"X_pred_{labelB}"], label=f"{labelB} X_pred")
        plt.plot(
            X_comb["t_s"], X_comb[f"X_Avrami_{labelB}"],
            linestyle="--",
            label=f"{labelB} Avrami (n={n:.2f}, X_inf={Xinf_B:.2f}, K={K_B:.3g})",
        )
        plt.xlabel("t (s)")
