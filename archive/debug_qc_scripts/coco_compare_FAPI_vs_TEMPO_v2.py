# coco_compare_FAPI_vs_TEMPO_v2.py
...
def mask_from_annotation(ann, H=None, W=None):
    ...
    if isinstance(seg, dict) and "size" in seg and "counts" in seg:
        ...
        if isinstance(counts, list):
            ...
            # compressed RLE requires pycocotools; skip gracefully
            return None, H, W
...
def load_coco_many(
    ...
    for jp in jsons:
        ...
        # collect per-image nuclei and defects
        nuclei_pts = {}
        defects_union = {}
        size_cache = {}
        ...
        for ann in anns:
            ...
            if is_nucleus:
                ...
                if k and len(k)>=2:
                    ...
                    # centroid fallback if it comes as small mask
                    try:
        ...
        for ann in anns:
            ...
            if cname and name_matches(cname, crystal_names): is_crystal=True
            # if nothing matched, treat any mask-like segmentation as potential crystal
...
def main():
    ...
    # configurable categories by ID and/or name (case-insensitive contains)
    ...
    # rank-to-time nucleation (0..60 ms)
    ...
    # parameters summary
    rows=[]
    def row(label,res,v0,aa,bb,K):
        d={"dataset":label,"v0_px_per_ms":v0,"alpha":aa,"beta":bb,"K":K,"I_model":res["model"]}
        if res["model"]=="gamma":
            d.update({"k":res["k"],"theta":res["theta"],"AIC":res["AIC"],"BIC":res["BIC"]})
        else:
            d.update({"mu":res["mu"],"sigma":res["sigma"],"AIC":res["AIC"],"BIC":res["BIC"]})
        return d
    rows.append(row(labelA,resA,v0A,aA,bB:=bB if 'bB' in locals() else bA,K_A))  # ensure variables exist
    rows[-1]["beta"]=bA
    rows.append(row(labelB,resB,v0B,aB,bB,K_B))
